...
describe("AuthController", () => {
	...
	describe("register", () => {
		...
		it("re-renders the registration page when required fields are missing", async () => {
			const authApiService = { register: vi.fn() };
			const controller = new AuthController(authApiService as never);

			const req = {
				body: { email: "newuser@kainos.com" },
				session: {},
			} as unknown as Request;
			const render = vi.fn();
			const res = { render } as unknown as Response;
		...
		it("re-renders the registration page when passwords do not match", async () => {
			const authApiService = { register: vi.fn() };
			const controller = new AuthController(authApiService as never);
		...
		it("re-renders the registration page when confirmation is missing", async () => {
			const authApiService = { register: vi.fn() };
			const controller = new AuthController(authApiService as never);
